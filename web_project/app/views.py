...
def deactivateStudent():
    ...
    if form.validate_on_submit():
        Student.query.get(form.student_id.data).active = False
        try:
            db.session.commit()
            flash(f'New Student added:  received', 'success')
            return redirect(url_for('index'))
        except:
            db.session.rollback()

    return render_template('deactivateStudent.html', title='deactivate', form=form)